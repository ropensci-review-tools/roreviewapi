...
function (u) {
    ...
    visjs_url <- paste0 (Sys.getenv ("pkgcheck_url"), "/assets/", visjs_file)
...
function (u) {
    ...
    # pkgcheck_install_deps is called in get_gp_report
    gp <- pkgcheck::get_gp_report (u, local_repo)
...
function (u) {
    ...
    chk <- pkgcheck::editor_check (local_repo, u)
...
function (u) {
    ...
    ec <- pkgcheck::pkgcheck (local_repo)
    ec <- editor_check (local_repo)

    srr <- pkgcheck::pkgcheck_srr_report (local_repo)
    ...
    if (srr_okay) {
        ...
        gp <- process_gp (gp, control = control)
        ...
        check <- pkgcheck::editor_check (local_repo, u)
        is_noteworthy <- attr (check, "is_noteworthy")

        if (any (grepl (pkgcheck::symbol_crs (), check))) {
